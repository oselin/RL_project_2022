...
class DDPG(object):
    def __init__(self, state_shape, action_dim, max_action, lr, gamma, tau, batch_size, buffer_size=1e6, grad_clip_norm = 10):
        state_dim = state_shape[0]
        self.action_dim = action_dim
        self.max_action = max_action
        self.pi = Policy(state_dim, action_dim, max_action).to(device)
    ...
    def _update(self,):
        ...
        #        3. compute actor loss and update the pi's parameters
        #        4. update the target q and pi using h.soft_update_params() (See the() DQN code)
        Q_target = batch.reward + batch.not_done*self.gamma*self.q_target(batch.next_state, self.pi_target(batch.next_state))
        q_loss = torch.mean(torch.square(Q_target.detach() - self.q(batch.state, batch.action)))
        q_loss.backward()

        torch.nn.utils.clip_grad_norm_(self.q.parameters(), self.grad_clip_norm)
        self.q_optim.step()

        pi_loss = -self.q(batch.state, self.pi(batch.state)).mean()
        pi_loss.backward()

        torch.nn.utils.clip_grad_norm_(self.pi.parameters(), self.grad_clip_norm)
        self.pi_optim.step()
